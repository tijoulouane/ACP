...
```{r}
fviz_pca_var(pca_blocs, col.var = "contrib")
fviz_pca_ind(pca_blocs, col.ind = "cos2")
```


Concernant les département ayant des profils atypiques ils sont toujours visible sur le graphiques des individus 
...
On remarque sur ce graphe que trois groupes sont bien éloignés des autres départements. On va donc décider de les supprimer car à eux seuls ils "attirent" la dimension 1, empêchant ainsi de bien visualiser les différentes informations.
...
```{r}
variables_quantitatives_ratios <- c("Biomasse", "Energie","Industrie","Résidentiel","Routier","Tertiaire","Déchets")
...
bartlett_result <- cortest.bartlett(
  cor(Base_dep_ratios[, variables_quantitatives], use = "pairwise.complete.obs"),
  n = nrow(Base_dep)
)
bartlett_result

pca_14 <- PCA(Base_dep[, variables_quantitatives], scale.unit = TRUE, graph = FALSE)
```
...
On conserve uniquement les deux premières dimensions.
...
Les deux premières dimensions permettent de capter presque trois quarts des informations pertinentes, ce qui est suffisant.
...
bartlett_result <- cortest.bartlett(
  cor(Base_dep_habs[, variables_quantitatives], use = "pairwise.complete.obs"),
  n = nrow(Base_dep)
)
bartlett_result
```

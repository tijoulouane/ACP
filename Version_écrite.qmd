...
corr_mat=cor(mydata,method="pearson")
...
# Pour chaque secteur, garder les 10 départements les plus émetteurs
Base_top10 <- Base_long %>%
  group_by(variable) %>%
  slice_max(value, n = 10) %>%  # top 10 par secteur
  ungroup()

# Boxplots
ggplot(Base_top10, aes(x = reorder(dep_nom, value), y = value, fill = variable)) +
  geom_boxplot() +
  coord_flip() +
  facet_wrap(~ variable, scales = "free") +  # un graphique par secteur
  labs(title = "Top 10 départements – Boxplots des émissions par secteur",
       x = "Département", y = "Émissions (tonnes CO₂ eq)")

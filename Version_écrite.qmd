...
[^2]: https://driaaf.ile-de-france.agriculture.gouv.fr/les-espaces-agricoles-representent-2-5-du-territoire-de-paris-et-de-la-proche-a3478.html
...
Le même tableau est disponible en annexe sur les émissions de C02 par secteur mais au niveau communale (Tableau 1)
...
Pour analyser les relations entre les variables quantitatives du jeu de données, soit les différents secteurs d’émissions de CO₂, nous utilisons le coefficient de corrélation de Pearson, puisque la matrice de corrélation servira par la suite à l’Analyse en Composantes Principales(voir l'enssemble des graphiques de la corrélation de pearson en annexe graphiques 6).
...
Ce graphique permet d’identifier visuellement les départements et secteurs les plus émetteurs, mais afin de voir cela de manière plus détaillée, en annexe (graphiques 11) se trouve le détail de chaque secteur, permettant d'observer réellement quels sont les départements les plus émetteurs de CO2 dans chaque secteur. En parallèle, le Top 10 des départements les plus émetteurs de CO2 est observable en annexe (graphiques 13).
...
Afin de vérifier l’adéquation de nos données à une Analyse en Composantes Principales, nous avons calculé l’indice KMO (Résultat test 1 en annexe). Celui-ci permet d’évaluer si les corrélations entre variables sont suffisamment structurées pour être synthétisées par une analyse factorielle.
...
Les tests de normalité de Shapiro-Wilk (Résultat test 2 en annexe) indiquent que toutes les variables sont significativement non normales (p-value \< 0,05), ce qui signifie qu’elles ne suivent pas une loi normale.

Nous avons quand même réalisé le test de Bartlett (Résultat test 3 en annexe) pour vérifier la présence de corrélations suffisantes entre les variables. Ce test est très significatif, et permet de rejeter l’hypothèse nulle d’indépendance entre les secteurs et confire que des corrélations importantes existent. Cependant, étant donné l’asymétrie des données, ce test n'est pas fiable. La justification de l'ACP se fait donc essentiellement sur l’indice KMO.
...
Ensuite, nous vérifions le déterminant de la matrice de corrélation de Pearson (Résultat test 4 en annexe), qui est très faible (\~6,67e-9). Cela indique que certaines variables sont très fortement corrélées, mais pas parfaitement (comme observé dans l’analyse bivariée). Cette situation confirme que l’ACP est appropriée pour synthétiser l’information contenue dans les différents secteurs.
...
L’analyse des contributions montre que la première dimension est principalement portée par les secteurs Biomasse, Résidentiel, Tertiaire et Routier. Cette observation est confirmée graphiquement (voir Graphique 16 en annexe), confirmant l’existence d’un bloc fortement corrélé puisque les axes son proche sur le plan. La dimension 1 représente donc un axe pouvant être appelé énergie domestique, tertiaire et mobilité. Le Cos² de ces variables sur Dim 1 est très élevé (Routier : 0,942 ; Biomasse : 0,891 ; Résidentiel : 0,870 ; Tertiaire : 0,871), ce qui indique qu’elles sont très bien représentées sur cette dimension et que Dim 1 reflète fidèlement ce bloc. Graphiquement, les flèches de ces variables sont proches et orientées dans la même direction, illustrant leur forte corrélation.

La deuxième dimension est dominée par les secteurs Énergie et Industrie, la dimension 2 représente donc l'axe secteur industriel. Cette observation est confirmée graphiquement (voir Graphique 17 en annexe). Le Cos² de ces variables sur Dim 2 est relativement élevé (Énergie : 0,619 ; Industrie : 0,538), confirmant qu’elles sont bien représentées sur Dim 2. Les flèches correspondantes sont proches entre elles mais séparées du bloc résidentiel/tertiaire, ce qui souligne leur comportement spécifique.

Les secteurs Agriculture et Déchets structurent principalement des dimensions secondaires que nous n’avons pas retenues pour l’analyse (Agriculture Cos² Dim 3 : 0,863 ; Déchets Cos² Dim 4 : 0,360), indiquant un comportement plus spécifique et reflétant moins la dynamique générale des principaux blocs sectoriels. Sur le plan, la flèche Agriculture pointe dans la direction opposée au bloc Routier/Biomasse/Résidentiel/Tertiaire, illustrant un comportement différent et une corrélation négative. Cependant, la faible longueur de la flèche du secteur Agriculture indique que cette opposition n’est pas très significative sur le plan, ce qui explique que la corrélation négative reste faible.

\underline{Graphique 18} : Projection des départements sur le plan 
```{r,.width=3, fig.height=3}
fviz_pca_ind(pca_full, col.ind = "cos2")+ ylim(-6.5, 13) + xlim(-3, 17)
```
...
Nous avons recalculé l’indice KMO (Résultat test 5 en annexe), qui est supérieur à 0,6, indiquant que les corrélations entre variables sont suffisante pour réaliser l'ACP. De plus, le déterminant de la matrice de corrélation (Résultat test 6 en annexe) est bien différent de zéro (0,412), contrairement à la première ACP où il était très proche de zéro, signe d’une forte multicolinéarité. Ces résultats confirment que l’ACP sur les blocs peut être réalisée.
...
# Graphique 18 : Eboulis des valeurs propres pour l'ACP après regroupement des variables fortement corrélées
...
La première composante principale explique 53 % de la variance, et les deux premières composantes cumulées expliquent 76,7 % de la variance totale. Les composantes 3 et 4 apportent moins de 23 % supplémentaire, ce qui montre que l’essentiel de l’information est capturé par les deux premières composantes. Cette conclusion se confirme par l’éboulis des valeurs propres (voir Graphique 18 en annexe). Selon la règle de Kaiser, seule la première composante devrait être retenue, car sa valeur propre est supérieure à 1. Cependant, la deuxième composante possède une valeur propre proche de 1 (0,95) et, en la conservant, l’ACP permet de capturer une inertie totale supérieure à 53 %. Nous avons donc décidé de retenir les deux premières composantes pour l’analyse.
...
Cependant, il faut prendre en compte les limites de cette représentation: à part les départements très émetteurs ou complètement agricoles, on ne parvient pas à visualiser les différences entre deux départements car la plus part se retrouvent très proche sur le graphe. Et comme précédemment, retiré les extrêmes n'apporterait rien car d'autres tireraient alors l'axe 1 à eux (70, 60...) et il faudrait ainsi répéter la procédure de nombreuses fois pour étudier deux départements précis.
...
Afin de vérifier nos résultats et éventuellement affiner nos conclusions, nous avons réalisé une nouvelle ACP, cette fois en regroupant les départements par région.
Sur le graphique des individus, cela confirme que les départements du Grand Ouest sont fortement caractérisés par des émissions liées à l’agriculture, puisque l’on observe que les régions Pays de la Loire, Normandie et Bretagne sont tirées vers ce type d’émissions.
...
On n'a également testé une ACP sur les émissions par habitants [^2] qui n'a rien donné de nouveau. On retrouverais à peu près la même spécification des axes et donc les mêmes départements étaient présents au extrêmes tandis que tout les autres étaient concentrés en un amas.

[^2]:(voir annexe: ACP_ind)

Globalement on n'arrive pas à en tirer un quelconque résultat de cette acp vu que seuls quelques département tirent vers eux tout l'axe 1. Une solution est donc de créer une nouvelle colonne mesurant les émissions totales du département pour ensuite déterminer un ratio de chaque département pour chaque secteur. On peut ainsi reprendre la base initiale, sans les départements "extrêmes".

## ACP sur les ratios d'émissions

Avec cette ACP, on ne cherche plus quel secteur est le plus émetteur. On va plutôt chercher à comprendre le profil de chaque département. C'est à dire, déterminer si le secteut a une tendance industrielle tout en étant peuplé ou bien si toutes ses émissions se concentrent uniquement sur le secteur résidentiel... On peut donc avoir des profils très diverses de mis en avant, permettant de mieux comprendre le dynamique de chacun.
...
```{r}
mydata <- Base_dep_ratios[,c("Agriculture","Biomasse","Déchets","Energie","Industrie","Résidentiel","Routier","Tertiaire")]
corr_mat_pearson=cor(mydata,method="pearson")
corrplot(corr_mat_pearson, method = 'number',type="upper",number.cex = 0.5)
graph_corr_number_pearson <- recordPlot()
det(corr_mat_pearson)
```
le déterminant étant très faible, on décide de supprimer les variables apportant une redondance dans l'information.
...
# tests justificatifs de l'acp

```{r}
variables_quantitatives_ratios <- c("Routier","Biomasse", "Energie","Industrie","Résidentiel","Déchets")

kmo_result <- KMO(Base_dep_ratios[, variables_quantitatives_ratios])
kmo_result

pca_14 <- PCA(Base_dep_ratios[, variables_quantitatives_ratios], scale.unit = TRUE, graph = FALSE)
```

Ici le seul moyen d'avoir un KMO supérieur à 0.5 est en retirant la variable agriculture, qui n'est pas corrélée positivement avec les autres.
Ainsi, on obtient un déterminant de 0.35.
```{r}
mydata <- Base_dep_ratios[,c("Routier","Biomasse","Déchets","Energie","Industrie","Résidentiel")]
corr_mat_pearson=cor(mydata,method="pearson")
det(corr_mat_pearson)
```
...
On conserve uniquement les deux premières dimensions car elles sont supérieures à 1 mais il semblerait que la troisième apporte une précision non négligeable.
...
Le graphique vient confirmer les résultats précédents, seule 60% de l'information est comprise dans les deux premières dimensions. Cependant le reste des valeurs propres chute donc la règle de kaiser est respectée. On concervera donc deux dimensions pour cette acp.

```{r}
pca_r <- PCA(Base_dep_ratios[, variables_quantitatives_ratios], scale.unit = TRUE, graph = FALSE)
...
fviz_contrib(pca_r, choice = "var", axes = 1, top = 10)
fviz_contrib(pca_r, choice = "var", axes = 2, top = 10)
pca_r$var$cos2
```
```{r}
fviz_pca_ind(pca_r,  
...
\underline{Graphique 18} : Eboulis des valeurs propres pour l'ACP après regroupement des variables fortement corrélées
```{r}
Eboulis_blocs
```
\underline {ACP_sans_extrêmes}: ACP groupée sans les extrêmes
```{r}
rownames(Base_dep_blocs_corrélé) <- Base_dep_blocs_corrélé$dep_code
dep_atypiques <- c("95", "94", "93", "76", "13")
Base_dep_blocs_corrélé <- Base_dep_blocs_corrélé[!Base_dep_blocs_corrélé$dep_code %in% dep_atypiques, ]
```

```{r}
pca_blocs_2 <- PCA(Base_dep_blocs_corrélé[, variables_quantitatives_blocs], scale.unit = TRUE, graph = FALSE)
```

```{r}
fviz_pca_var(pca_blocs_2, 
                   col.var = "contrib", 
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                   repel = TRUE,
                   title = "Cercle des corrélations - Variables")
...
\underline {ACP_ind} : ACP émissions par individus
```{r}
#| message: false

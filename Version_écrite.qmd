...
```
Ici, l'axe principal concentre les 3 blocs principaux, ce qui indique que plus un département sera sur la droite plus ce sera un département émetteur (tous secteurs confondus). L'axe 2 lui est caractérisé par l'agriculture, ce qui indique qu'un département placé "plus haut" possédera une composante agricole importante dans ses émissions totales.
```{r}
fviz_pca_ind(pca_blocs, col.ind = "cos2")
```
De manière logique, on retrouve les mêmes départements que précédemment, avec le 13 qui se situe en haut, de par sona ctivité agricole non négligeable (élevage bovin, maraîchage, rizicultures...). 
...
Cependant, il faut prendre en compte les limites de cette représentation: à part les départements très émetteurs ou agricoles, on ne parvient pas à visualiser les différences entre deux départements car la plus part se retrouvent très proche sur le graph. Et comme précédemment, retiré les extrêmes n'apporterait rien car d'autres tireraient alors l'axe 1 à eux (70, 60...) et il faudrait ainsi répéter la procédure de nombreuses fois pour étudier deux départements précis.

```{r}
# --- Analyse des groupes ---
res.hcpc <- HCPC(pca_full, graph = FALSE)
...
```{r}
pca22 <- PCA(Base_dep_blocs_corrélé[, variables_quantitatives_blocs], scale.unit = TRUE, graph = FALSE)
```
...
```
On observe une légère modification dans la spécification des axes. 
...
On remarque qu'en enlevant les plus atypiques, d'autres apparaissent. Ce processus se répète de par la présence de départements plus spécifiés que d'autres dans certains domaines.
...
Les différentes méthodes précédentes
...
Ici le seul moyen d'avoir un KMO supérieur à 0.5 est en retirant la variable agriculture, qui n'est pas corrélée avec les autres.
...
Le graphique vient confirmer les résultats précédents, seul 60% de l'information est comprise dans les deux premières dimensions. Cependant le reste des valeurs propres chute donc la règle de kaiser est respectée. On concervera donc deux dimensions pour cette acp.
...
A l'inverse, l'axe 2 semble montrer la nature du mode de vie avec une opposition entre les variables déchets/résidentiel et Biomasse. Cet axe permet donc d'indiquer si le département est urbanisé ou possède encore des ressources naturelles. 
```{r}
fviz_pca_ind(pca_14,  
                   col.ind = "cos2", 
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
```{r}
kmo_result <- KMO(Base_dep_habs[, variables_quantitatives])
kmo_result
```

```{r}
pca_16 <- PCA(Base_dep_habs[, variables_quantitatives], scale.unit = TRUE, graph = FALSE)
...
```{r}
dep_atypiques_habs <- c(13, 70,77,60,45,70,76,93,94,95)
base_dep_habs <- Base_dep_habs[!Base_dep_habs$dep_code %in% dep_atypiques_habs, ]

pca_15 <- PCA(base_dep_habs[, variables_quantitatives], scale.unit = TRUE, graph = FALSE)
...
On observe le même cas que tout à l'heure avec quelques département aux extrêmes qui attirent à eux la dimensions 1. Cette méthode ne va donc pas pouvoir nous apporter grand chose mais on remarque malgré tout que ce sont les mêmes départements qui ressortent en tant que "valeur atypique".

...
```{r}
Base_dep_finale_2 <- Base_dep %>%
  mutate(Vie_humaine = Résidentiel + Tertiaire + Routier) %>%
  select(-Résidentiel, -Tertiaire, -Routier)
```

```{r}
variables_quanti_finales_2 <- c("Agriculture","CO2_biomasse","Déchets", "Energie","Industrie","Vie_humaine")  
```

```{r}
kmo_result <- KMO(Base_dep_finale_2[, variables_quanti_finales_2])
kmo_result
...
bartlett_result <- cortest.bartlett(
  cor(Base_dep_finale_2[, variables_quanti_finales_2], use = "pairwise.complete.obs"),
  n = nrow(Base_dep_finale_2)
)

bartlett_result
```
```{r}
rownames(Base_dep_finale_2) <- Base_dep_finale_2$dep_code
```

```{r}
mydata <- Base_dep_finale_2[, c("Agriculture","CO2_biomasse","Déchets","Energie","Industrie","Vie_humaine")]
chart.Correlation(mydata, histogram=TRUE, pch=19,method = c("spearman"))
...
```{r}
pca5 <- PCA(Base_dep_finale_2[, variables_quanti_finales_2], scale.unit = TRUE, graph = FALSE)
pca5$eig
```
...
```{r}
base_dep_finale_clean_2 <- Base_dep_finale_2[!Base_dep_finale_2$dep_code %in% dep_atypiques, ]
```

```{r}
pca6 <- PCA(base_dep_finale_clean_2[, variables_quanti_finales_2], scale.unit = TRUE, graph = FALSE)
```

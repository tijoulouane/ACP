...
# A. Justification du nombre d'axes (Scree Plot)
p1 <- fviz_eig(pca, addlabels = TRUE, main = "Eboulis des valeurs propres")

plot(p1)
```

```{r}
base_sans_extremes <- Base_dep[-c(95,94,93,76,13),]
pca2 <- PCA(base_sans_extremes[, variables_quantitatives], 
            scale.unit = TRUE, 
            graph = FALSE)
# B. Cercle des corrélations (Variables)
# Les couleurs indiquent la contribution (quelles variables créent les axes)
p2 <- fviz_pca_var(pca2, 
                   col.var = "contrib", 
...
res.hcpc <- HCPC(pca2, graph = FALSE)
...
res_vars <- pca2$var$contrib
head(res_vars[order(res_vars[,1], decreasing = TRUE), ])
```

```{r}
res.desc <- dimdesc(pca2, proba = 0.05)
...
```{r}
fviz_pca_var(pca_clean, col.var = "contrib")
```
...
```{r}
base_sans_extremes_2 <- base_dep_sans_Agriculture[-c(95,94,93,76,13),]
pca3 <- PCA(base_sans_extremes_2[, variables_quantitatives_sans_Agriculture], 
            scale.unit = TRUE, 
            graph = FALSE)
# B. Cercle des corrélations (Variables)
# Les couleurs indiquent la contribution (quelles variables créent les axes)
p2 <- fviz_pca_var(pca3, 
                   col.var = "contrib", 
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                   repel = TRUE,
                   title = "Cercle des corrélations - Variables")
...
on obtient pas pareil a cause de repel 

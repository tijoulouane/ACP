...
batiments en plus et industrie en moins
...
on a une corrélation supérieure à 95% entre résidentiel, routier et tertiaire. Ce qui est tout à fait normal pour résidentiel et tertiaire car ils sont liés à la densité de population humaine, là où il y a du travail, il y a des logements. On va donc opter pour un regroupement de ces deux variables sous une seule: bâtiments.

```{r}
Base_dep_finale_3 <- Base_dep %>%
  mutate(Batiments = Résidentiel + Tertiaire) %>%
  select(-Résidentiel, -Tertiaire, -Energie)
```

```{r}
variables_quanti_finales_3 <- c("Agriculture","CO2_biomasse","Déchets", "Industrie","Batiments")  
```

```{r}
kmo_result <- KMO(Base_dep_finale_3[, variables_quanti_finales_3])
kmo_result
...
bartlett_result <- cortest.bartlett(
  cor(Base_dep_finale_3[, variables_quanti_finales_3], use = "pairwise.complete.obs"),
  n = nrow(Base_dep_finale_3)
)
...
```{r}
rownames(Base_dep_finale_3) <- Base_dep_finale_3$dep_code
```

```{r}
mydata <- Base_dep_finale_3[, c("Agriculture","CO2_biomasse","Déchets","Batiments","Industrie")]
chart.Correlation(mydata, histogram=TRUE, pch=19,method = c("spearman"))
...
```{r}
pca7 <- PCA(Base_dep_finale_3[, variables_quanti_finales_3], scale.unit = TRUE, graph = FALSE)
pca7$eig
```

```{r}
fviz_pca_var(pca7, col.var = "contrib")
```

```{r}
fviz_pca_ind(pca7, col.ind = "cos2")
```

```{r}
base_dep_finale_clean_3 <-  Base_dep_finale_3[!Base_dep_finale_3$dep_code %in% dep_atypiques, ]
```

```{r}
pca8 <- PCA(base_dep_finale_clean_3[, variables_quanti_finales_3], scale.unit = TRUE, graph = FALSE)
```

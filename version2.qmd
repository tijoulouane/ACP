...
```
```{r}
res.hcpc <- HCPC(pca4, graph = FALSE)
...
## acp sur groupes
```{r}
Base_clusters <- res.hcpc$data.clust

# ACP pour le Groupe 1
data_g1 <- Base_clusters[Base_clusters$clust == 1, variables_quanti_finales]
# On retire les colonnes constantes pour éviter l'erreur "times incorrect"
data_g1 <- data_g1[, sapply(data_g1, function(x) var(x, na.rm=TRUE) > 0)]
acp_g1 <- PCA(data_g1, graph = FALSE)
fviz_pca_var(acp_g1, 
                   col.var = "contrib", 
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                   repel = TRUE,
...
# ACP pour le Groupe 2
data_g2 <- Base_clusters[Base_clusters$clust == 2, variables_quanti_finales]
data_g2 <- data_g2[, sapply(data_g2, function(x) var(x, na.rm=TRUE) > 0)]
acp_g2 <- PCA(data_g2, graph = FALSE)
fviz_pca_var(acp_g2, 
                   col.var = "contrib", 
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                   repel = TRUE,
